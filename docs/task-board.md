...
| E4 | Accepted — UNTESTED live | Orchestrator | ElevenLabs Settings & Keyring UI wiring | 13 focused TUI/runtime tests; provider-namespaced keyring storage (`ELEVENLABS_API_KEY`), dynamic model switcher in Settings, masked credential input/removal, and fail-closed UNTESTED live protection |
| E5 | In progress — UNTESTED live | Orchestrator | ElevenLabs chunker & coordinator wiring | Reconcile character limits with EPUB chunker; per-provider rate limits; audio cache collision isolation |
